...
async def do_draw(event: Event, uid: int, wallet: UserWallet, user_text: str) -> None:
    ...
    await draw_cmd.send(f"正在画「{user_text}」…\n已扣除100000金币")
    ...
    try:
        ...
        image_bytes = await generate_image(
            _config["gpt_image_api_key"], _config["gpt_image_aimodel_list_id"], english_prompt
        )
...
async def do_edit(event: Event, uid: int, wallet: UserWallet, user_text: str) -> None:
    ...
    await edit_cmd.send(f"正在修图「{prompt}」…\n已扣除 10万 金币 (剩余 {wallet.gold})")
    ...
    try:
        ...
        image_bytes = await generate_image_edit(
            _config["gpt_image_api_key"], _config["gpt_image_aimodel_list_id"], english_prompt, ref_image
        )
        image_msg = build_image_msg(event, image_bytes)
